...
The sample average of μ_hat across tests for which the null is rejected is NOT approximately equal to the true value of μ when μ = 1,2 and 3. Since sample mean is far away from the mean under the null hypothesis when the sample mean is relatively low, and the average of mean of the rejected tests is far away from the null too.From the power_plot, we can see for μ = 1,2 and 3, the power of test is pretty low (0.2~0.8).

However, when μ = 4,5 and 6, the rejected estimate is approximately equal to the true value of μ since now almost all tests are rejected. From the above power_plot, we can see for μ = 4,5 and 6, the power of test is around 1.

When μ = 0, the rejected samples have their estimated μ close to their true mean = 0, which is exactly the null hypothesis. Thus, the rejected samples’ estimated μ is still close to the true mean, regardless of its low power.

From the above 3 plots we know, as effect size increases, power increases and approach to 1, and the estimated μ will approach the true population μ as well.
